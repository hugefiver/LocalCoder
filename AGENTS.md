...
- Browser debugging/QA launches the browser via CDP (`--remote-debugging-port` on loopback) with a throwaway `--user-data-dir` profile under the OS temp area: no extensions, no signed-in accounts, no settings sync, never the default profile. Launch the browser and any dev/preview server as detached background processes with output redirected to log files so the shell tool is never blocked. The acceptance criterion for any background launch is that the shell invocation which started it exits normally and returns the PID immediately; a command is invalid if the shell stays occupied, regardless of a background/detached flag. A launch shell call must only start the process, persist/print its PID, and return immediately; split readiness checks, bounded waits, browser automation, log reads, and cleanup into separate shell calls with explicit timeouts. Every shell tool call that may wait must set a finite timeout appropriate to the operation; a timeout is a failure signal that requires ownership inspection and cleanup, never a reason to keep polling indefinitely or treat the operation as successful. Do not assume PowerShell `Start-Process` is detached from the shell tool host: if the host tracks its descendant and keeps the call open, use a short Node launcher with `child_process.spawn({ detached: true, stdio: ["ignore", outputFd, errorFd] }).unref()` or `Win32_Process.Create`, then verify the recorded PID and command line in a separate call. Never run a foreground server/browser, `Wait-Process` on a long-lived process, or a combined launch-and-wait command. Terminate only the processes/ports the QA run started and delete its temp profile afterward.
