...
(() => {
  ...
  async function execute(source, outputBytes) {
    ...
    try {
      const value = await captureConsole(output, async () => {
        ...
        return result === void 0 ? null : canonicalResult(result);
      });
  ...
  async function judge(source, cases, outputBytes) {
  ...
  async function judgeCase(source, testCase, budget) {
  ...
  function compileFreeSource(source) {
    try {
...
${source}`);
  ...
  function compileJudgeSource(source) {
    try {
      return new Function(
        "input",
        "missing",
...
return typeof solution === "function" ? solution(input) : missing;`
  ...
  async function captureConsole(output, operation) {
...
`);
    ...
    consoleObject.log = (...values) => write("stdout", values);
    consoleObject.info = (...values) => write("stdout", values);
    consoleObject.debug = (...values) => write("stdout", values);
    consoleObject.warn = (...values) => write("stderr", values);
    consoleObject.error = (...values) => write("stderr", values);
    try {
  ...
  function transpileTypeScript(compiler, source) {
    ...
    if (errors.length > 0) {
      ...
      if (first === void 0) throw new Error("TypeScript diagnostic list unexpectedly lost its first error");
      return {
        ok: false,
  ...
  function createTypeScriptRuntime(compiler, options = {}) {
    const evaluator = options.evaluator ?? createJavaScriptRuntime({
      ...options.buildId === void 0 ? {} : { buildId: options.buildId },
      ...options.outputBytes === void 0 ? {} : { outputBytes: options.outputBytes }
    });
    return {
      initialize: async () => {
  ...
  function createTypeScriptAssetRuntime(scope, options = {}) {
    let runtime;
  ...
  function loadTypeScriptCompiler(scope) {
    try {
  ...
  function emittedCode(compiler, source) {
  ...
  function emittedFreeCode(compiler, source) {
...
return ${wrapperName}();`;
  ...
  function diagnosticLocation(diagnostic) {
    if (diagnostic.file === void 0 || diagnostic.start === void 0) return "";
  ...
  function boundedDiagnostics(diagnostics) {
    ...
    for (const diagnostic of diagnostics) {
      const delimiterBytes = bounded2.length === 0 ? 0 : 1;
      const available = MAX_DIAGNOSTIC_BYTES - bytes - delimiterBytes;
      if (available <= 0) break;
      const text = truncateUtf82(diagnostic, available);
      if (text.length === 0) break;
      bounded2.push(text);
      bytes += delimiterBytes + encoder3.encode(text).byteLength;
    }
    return bounded2;
  }
  function truncateUtf82(text, limit) {
    ...
    let result = "";
    for (const codePoint of text) {
      const codePointBytes = encoder3.encode(codePoint).byteLength;
      if (bytes + codePointBytes > limit) break;
      result += codePoint;
      bytes += codePointBytes;
    }
  ...
  function infrastructureFailure(code, message) {
    return { kind: "infrastructure", code, message, fatal: true };
  }
  function isTypeScriptCompiler(value) {
    if (value === null || typeof value !== "object") return false;
    const candidate = value;
    return typeof candidate.version === "string" && candidate.version.trim().length > 0 && typeof candidate.transpileModule === "function" && typeof candidate.flattenDiagnosticMessageText === "function" && hasNumber(candidate.DiagnosticCategory, "Error") && hasNumber(candidate.ModuleKind, "None") && hasNumber(candidate.ScriptTarget, "ES2020");
  }
  function hasNumber(value, key) {
    return value !== null && typeof value === "object" && typeof value[key] === "number";
  }
  ...
  function selectedEndpoint(event) {
    if (endpoint !== void 0) return endpoint;
    endpoint = runtimeIdOf(event.data) === "typescript-official" ? createWorkerEndpoint({
  ...
  function runtimeIdOf(value) {
    if (value === null || typeof value !== "object") return void 0;
    const runtimeId = value.runtimeId;
    return typeof runtimeId === "string" ? runtimeId : void 0;
  }
})();
